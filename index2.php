...
$sql_check = "SELECT book, chapter, paragraph, sentence FROM daily_verse WHERE date = '$today_date'";
...
<div class="container">
    ...
    <section class="qt-section">
        ...
        <?php if ($is_logged_in): ?>
            <h3>📖 QT 작성</h3>
            <form id="qtForm">
                <textarea id="userQtContent" name="user_qt" placeholder="오늘의 말씀에 대한 QT를 작성하세요"><?= htmlspecialchars($user_qt) ?></textarea>
    ...
    <script>
    document.getElementById("qtForm")?.addEventListener("submit", function(event) {
        event.preventDefault();
        
        const qtContent = document.getElementById("userQtContent").value.trim();
        if (qtContent === "") {
            alert("QT 내용을 입력해주세요.");
            return;
        }
        ...
        fetch("save_qt.php", {
            ...
            body: new URLSearchParams({ user_qt: qtContent })
        })
